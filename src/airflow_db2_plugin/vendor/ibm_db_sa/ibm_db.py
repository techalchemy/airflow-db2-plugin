...
class DB2Dialect_ibm_db(DB2Dialect):
    ...
    colspecs = util.update_copy(
        DB2Dialect.colspecs, {sa_types.Numeric: _IBM_Numeric_ibm_db}
    )
    ...
    def is_disconnect(self, ex, connection, cursor):
        if isinstance(ex, (self.dbapi.ProgrammingError, self.dbapi.OperationalError)):
            return (
                "Connection is not active" in str(ex)
                or "connection is no longer active" in str(ex)
                or "Connection Resource cannot be found" in str(ex)
                or "SQL30081N" in str(ex)
            )
        else:
            return False
