...
class DB2Compiler(compiler.SQLCompiler):
    ...
    def visit_cast(self, cast, **kw):
        ...
        # for example
        if isinstance(
            type_, (sa_types.DateTime, sa_types.Date, sa_types.Time, sa_types.DECIMAL)
        ):
...
class DB2DDLCompiler(compiler.DDLCompiler):
    ...
    def create_table_constraints(self, table, **kw):
        if (self.dialect.dbms_name.find("DB2/") != -1) and (
            [int(ver_token) for ver_token in self.dialect.dbms_ver.split(".")[0:2]]
            >= [10, 5]
        ):
                ...
                if isinstance(constraint, sa_schema.UniqueConstraint):
                    ...
                    if hasattr(constraint, "use_alter") and constraint.use_alter:
                        ...
                        index = sa_schema.Index(
                            index_name,
                            *(column for column in constraint.columns._all_columns),
                        )
                        index.unique = True
        ...
        if (self.dialect.dbms_name.find("DB2/") != -1) and (
            ...
            if isinstance(create.element, sa_schema.UniqueConstraint):
                ...
                if getattr(create.element, "exclude_nulls", None):
                    ...
                    index = sa_schema.Index(
                        index_name,
                        *(column for column in create.element.columns._all_columns),
                    )
                    index.unique = True
                    index.exclude_nulls = True
...
class DB2ExecutionContext(default.DefaultExecutionContext):
    @property
    def statement(self):
        return self._statement.decode("utf-8")
