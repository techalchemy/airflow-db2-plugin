...
from sqlalchemy import Table, Column, MetaData, sql, util
...
class DB2Reflector(BaseReflector):
        ...
        for r in connection.execute(query):
            ...
            if coltype in ["DECIMAL", "NUMERIC"]:
                ...
                try:
                    ...
                    util.warn(
                        "Did not recognize type '%s' of column '%s'" % (coltype, r[0])
                    )
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        query = sql.select(
            [sysidx.c.indname, sysidx.c.colnames, sysidx.c.uniquerule],
            sql.and_(
                sysidx.c.tabschema == current_schema, sysidx.c.tabname == table_name
            ),
                ...
                indexes.append(
                    {
                        "name": self.normalize_name(r[0]),
                        "column_names": [
                            self.normalize_name(col) for col in col_finder.findall(r[1])
                        ],
                        "unique": r[2] == "U",
                    }
                )
